...
## Access the Live App
[Click Here to Open Application](https://prime-video-clone-deployment.vercel.app/)
...
pipeline {
    ...
    stages {
        ...
        stage ("Deploy to Conatiner") {
            steps {
                sh 'docker run -d --name amazon-prime -p 3000:3000 juhisinha/amazon-prime:latest'
            }
        }
    }
    post {
...
sudo chmod +x /usr/local/bin/argocd
   ...
   Reference Link: https://www.eksworkshop.com/docs/automation/gitops/argocd/access_argocd/?utm_source=chatgpt.com
   ```
...
kubectl apply -n argocd \
  -f https://raw.githubusercontent.com/argoproj/argo-cd/v2.4.7/manifests/install.yaml
...
## Configure ArgoCD (Expose via LoadBalancer)
```
# Expose ArgoCD server via LoadBalancer
kubectl patch svc argocd-server -n argocd \
  -p '{"spec": {"type": "LoadBalancer"}}'

# Wait ~2 minutes, then get the hostname
